...
## 2.0 设计与当前边界（2026-09-23）

已新增 [2.0 PRD 与配套规格](product/v2.0/README.md)，属于未来目标，不表示本轮实现了新执行能力。当前是有人指导的试点平台：组合器仍是内置能力目录，规划尚未消费完整业务上下文，探索受限，记忆与诊断尚未形成自主调查闭环。开放 SDK/MCP、自适应执行、复杂视觉交互、有效代码测试生成与真实业务检出评测需要新增工程。

本轮只调整仓库呈现、贡献入口和设计文档；1.0 历史测试数字不等同 2.0 验收。
