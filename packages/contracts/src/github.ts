...
export const GitHubCiConfig=z.object({enabled:z.boolean(),branch:z.string().min(1).max(200).refine(s=>!s.startsWith('/')&&!s.includes('..')&&!/[\s~^:?*\[\\]/.test(s)),templateId:z.string().min(1),kind:CodeCheckRequest.shape.kind,subdirectory:CodeCheckRequest.shape.subdirectory,timeoutSeconds:CodeCheckRequest.shape.timeoutSeconds,installDependencies:CodeCheckRequest.shape.installDependencies}).strict();
