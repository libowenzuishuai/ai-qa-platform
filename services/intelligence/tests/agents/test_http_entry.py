...
from agent_fixtures import ready_agent_pipelines, vector, VECTORS
...
@pytest.mark.parametrize("v", [v for v in VECTORS if not v["valid"]], ids=lambda v: v["name"])
def test_rejected_shared_vectors_at_default_http_entry(v, tmp_path):
    """All shared negatives go through the default pipeline and HTTP error envelope."""
    kind = v["kind"]
    entries = registered_entries(tmp_path, kind, v)
    with TestClient(create_app(token=TOKEN, gateway_factory=mock_gateway_factory(entries))) as client:
        response = post(client, "rules/extract" if kind == "rules" else "cases/generate", v)
    assert response.status_code == 422, response.text
    assert response.json()["code"] == "MODEL_OUTPUT_INVALID", response.text
    assert response.json()["requestId"] == "req-http-1"
