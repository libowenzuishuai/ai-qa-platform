...
export function caseEditor(c:any) {
  ...
  return `<h1>审阅与编辑用例</h1>${(c.changeReviews??[]).map((r:any)=>`<p><a href="/changes/${esc(r.id)}">${r.resolved?'查看已确认的需求变更':'需求来源已更新，查看待复核影响'}</a></p>`).join('')}<p>${esc(c.title)} · 版本 ${c.version} · ${c.approvalStatus==='APPROVED'?'已批准':'待审阅'}</p><p class="muted">保存会创建新草稿。已批准用例、历史运行及验收标准保持原样。</p>
  <form class="editor-form" method="post" action="/cases/${esc(c.id)}/revise">
  ...
  ${steps}${assertions}<section class="card"><h2>测试数据</h2>${choice('dataStrategy','准备方式',[['create','由用例步骤创建'],['fixture','使用已配置夹具']],c.dataSpec.strategy)}${area('dataNote','数据创建说明',c.dataSpec.note)}<p class="muted">在准备中心登记方案后，在这里选择。执行时会自动分配独立资源，并按清理策略收尾。</p>${choice('fixtureId','已登记的数据准备方案',[['','请选择方案'],...(c.availableDataPlugins??[]).map((p:any)=>[p.id,`${p.name} · 版本 ${p.version}`] as [string,string])],c.dataSpec.fixtureId)}</section>${params}
