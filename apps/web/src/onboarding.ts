...
export function onboarding(d:any) {
  ...
  return `<section class="card"><span class="muted">项目接入进度 · ${items.filter(i=>i.done).length} / ${items.length}</span><h2>${next?`下一步：${esc(next.title)}`:'基础接入已完成，可开始下一次验收'}</h2><p>${esc(next?.detail??'查看交付评估中的失败与未验证范围。')}</p><p><a class="primary-link" href="${next?.href??`${base}?tab=assessment`}">${next?'继续准备':'查看交付评估'} →</a></p><ol class="journey">${items.map(i=>`<li><span class="badge ${i.done?'PASS':'PENDING'}">${i.done?'已有记录':'待完成'}</span> <a href="${i.href}">${esc(i.title)}</a><p class="muted">${esc(i.detail)}</p></li>`).join('')}</ol><p class="muted">接入进度表示配置和资产记录，不代表业务已经通过。</p></section>`;
}
