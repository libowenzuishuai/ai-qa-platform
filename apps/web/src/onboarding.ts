...
export function onboarding(d:any) {
  ...
  const approvedCount=d.metrics?.approved??approved.length,latestCount=d.metrics?.latestCases??latest.length,boundCount=d.metrics?.bound??bound.length;
  const items=[
    ...
    {title:'审阅规则与用例',done:approvedCount>0,detail:`${approvedCount} 个最新用例已批准；${latestCount-approvedCount} 个待审阅`,href:`${base}?tab=missions#case-review`},
    {title:'观察页面并确认计划',done:boundCount>0,detail:`${boundCount} 个用例匹配当前环境；账号是否有效会在执行时验证`,href:`${base}?tab=missions#observe`},
    {title:'准备任务并开始验收',done:(d.metrics?.finishedRuns??d.executions.filter((r:any)=>r.lifecycle==='FINISHED').length)>0,detail:(d.totals?.missions??d.missions.length)?'先检查账号、数据与清理要求，再开始测试':'选择批准基线，创建第一项测试任务',href:`${base}?tab=missions#create-mission`},
  ];
