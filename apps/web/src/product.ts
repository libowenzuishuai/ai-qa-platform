...
export function registerProductPages(app:FastifyInstance){
  ...
  app.get('/space/:id',async(req,reply)=>{
    const sid=req.cookies.web_sid;if(!sid)return reply.redirect('/login');
    const {id}=req.params as {id:string}; const tab=String((req.query as any).tab??'overview');const page=Math.max(1,Math.min(100000,Number((req.query as any).page)||1));
    try{
      if(tab==='preparation')return reply.redirect(`/preparation/${id}`);
      if(tab==='workflows')return reply.redirect(`/workflows/project/${id}`);
      const {data:d}=await api<any>(`/api/projects/${encodeURIComponent(id)}/workspace?page=${page}`,{sid});
      ...
      <section class="card" id="case-review"><h2>1. 用例审阅</h2>${d.cases.map((c:any)=>`<article><h3><a href="/cases/${esc(c.id)}">${esc(c.title)}</a></h3><p>${esc(c.approvalStatus)} · ${c.plans.length?'已绑定计划':'等待执行计划'}</p>${c.approvalStatus==='DRAFT'?form('approve-case',`<input type="hidden" name="caseId" value="${esc(c.id)}">`,'批准用例'):''}</article>`).join('')}</section>
      <section class="card" id="observe"><h2>2. 观察真实页面</h2>${form('observe',envSelect()+field('role','业务角色','visitor')+field('paths','页面路径（逗号分隔）','/')+'<details><summary>登录或准备操作（高级设置）</summary><p>支持 fill/click 和凭据引用；仅执行负责人明确批准的准备动作。</p><textarea name="setup" rows="5" style="width:100%">[]</textarea></details><label><input type="checkbox" name="allowWrites" value="true">允许上述准备操作写入测试环境</label>','开始观察')}</section>
      ...
      <section class="card" id="create-mission"><h2>5. 创建任务</h2>${form('mission',field('title','任务名称')+field('goal','验收目标')+'<label>任务类型</label><select name="template"><option value="RELEASE">发布验收</option><option value="REGRESSION">变更回归</option><option value="HEALTH">项目体检</option></select>'+envSelect()+select('baselineId','批准基线',d.baselines),'创建任务')}</section>
      ${d.missions.map((m:any)=>`<section class="card"><h3>${esc(m.title)}</h3><p>${esc(m.goal)}</p><a class="primary-link" href="/missions/${esc(m.id)}/prepare">检查准备情况并开始 →</a></section>`).join('')}`;
      if(tab==='execution')body=`<h1>执行中心</h1><p>打开运行可查看步骤、实时事件、取消与证据。</p>${runList(d.executions)}`;
      if(tab==='assessment')body=`<h1>交付评估</h1>${d.executions.map((r:any)=>`<section class="card"><h3>${esc(r.buildId??'未声明版本')} · ${esc(r.acceptanceStatus)}</h3><a href="/runs/${esc(r.id)}/report">证据与报告</a>${['FINISHED','ERROR','CANCELLED'].includes(r.lifecycle)?form('retest',`<input type="hidden" name="runId" value="${esc(r.id)}">`+field('buildId','修复后的新构建版本'),'沿用原标准复测'):''}</section>`).join('')}<h2>缺陷跟踪</h2><a href="/projects/${esc(id)}/defects">查看全部缺陷、严重度与出现历史 →</a>${d.defects.map((f:any)=>`<section class="card"><h3>${esc(f.title)}</h3><p>${esc(f.status)} · 负责人 ${esc(d.members.find((m:any)=>m.userId===f.assignedTo)?.user.displayName??'未分配')} · ${f._count?.occurrences??f.occurrences.length} 次出现</p><pre>${esc(f.description)}</pre>${form('defect-update',`<input type="hidden" name="defectId" value="${esc(f.id)}"><select name="status"><option>CONFIRMED</option><option>FIX_PENDING</option><option>READY_FOR_RETEST</option><option>REJECTED</option></select>`+field('reason','处理说明')+select('assignedTo','负责人',d.members.map((m:any)=>({id:m.userId,name:m.user.displayName}))),'更新处理状态')}${form('defect-verify',`<input type="hidden" name="defectId" value="${esc(f.id)}">`+select('runId','用于确认修复的运行',d.executions,(r:any)=>`${r.buildId} · ${r.acceptanceStatus}`),'核验并确认修复')}</section>`).join('')}`;
      ...
      if(tab==='integrations') {
        const {data:code}=await api<any>(`/api/projects/${encodeURIComponent(id)}/code-checks?page=${page}`,{sid});
        d.totalPages=Math.max(d.totalPages,Math.ceil(code.total/code.pageSize));
        ...
        <section class="card"><h2>运行仓库现有测试或构建</h2><p>这类结果属于工程健康检查，不能替代业务验收。</p>${form('code-check',field('repositoryUrl','GitHub 仓库（私库需先连接）')+field('commitSha','固定提交（完整 40 位 SHA）')+'<label>仓库内子目录（可选）</label><input type="text" name="subdirectory"><label>执行方式</label><select name="kind"><option value="NODE_TEST">Node 内置测试</option><option value="PYTHON_TEST">Python pytest</option><option value="NODE_BUILD">Node 构建</option><option value="NODE_VITEST">Vitest</option><option value="NODE_JEST">Jest</option><option value="NODE_PLAYWRIGHT">Playwright</option><option value="NODE_LINT">ESLint</option><option value="NODE_TYPECHECK">TypeScript</option></select><label><input type="checkbox" name="installDependencies" value="true">允许在隔离准备容器中安装依赖</label><label><input type="checkbox" name="coverage" value="true">导入 LCOV 行覆盖报告</label>'+field('coveragePath','覆盖报告相对路径','coverage/lcov.info'),'创建工程检查')}</section>
        ${code.checks.map((c:any)=>`<section class="card"><h3>${esc(c.request.kind)} · ${esc(c.status)}</h3><p>工程检查：${esc(c.verdict)} · ${esc(c.request.commitSha)}</p>${c.summary?.coverage?`<p>行覆盖：${c.summary.coverage.linesHit} / ${c.summary.coverage.linesFound}（不代表业务需求覆盖）</p>`:''}${c.evidenceId?`<a href="/artifacts/${esc(c.evidenceId)}">下载受限结果证据</a>`:''}${['QUEUED','RUNNING'].includes(c.status)?form('cancel-check',`<input type="hidden" name="checkId" value="${esc(c.id)}">`,'取消'):''}</section>`).join('')}`;
      }
      ...
      if(tab!=='overview')body+=`<nav aria-label="项目列表翻页"><p>第 ${page}/${d.totalPages} 页 · 每类每页 ${d.pageSize} 条；共 ${d.totals.cases} 个用例版本、${d.totals.executions} 次运行、${d.totals.missions} 个任务。</p>${page>1?`<a href="?tab=${encodeURIComponent(tab)}&page=${page-1}">上一页</a>`:''} ${page<d.totalPages?`<a href="?tab=${encodeURIComponent(tab)}&page=${page+1}">下一页</a>`:''}</nav>`;
  ...
  for(const action of ['repository','environment','import','approve-case','observe','propose','baseline','mission','start','runtime','retest','defect-update','defect-verify','runner','revoke-runner','code-check','cancel-check','api-template'])app.post(`/space/:id/${action}`,async(req,reply)=>{
    ...
    try{
      ...
      if(action==='runner'){path=`/api/projects/${id}/runners`;body={name:b.name,capabilities:['NODE_TEST','PYTHON_TEST','NODE_BUILD','NODE_VITEST','NODE_JEST','NODE_PLAYWRIGHT','NODE_LINT','NODE_TYPECHECK','NODE_HTTP']};tab='integrations';}
      if(action==='revoke-runner'){path=`/api/runners/${b.runnerId}/revoke`;tab='integrations';}
      if(action==='code-check'){path=`/api/projects/${id}/code-checks`;body={repositoryUrl:b.repositoryUrl,commitSha:b.commitSha,subdirectory:b.subdirectory??'',kind:b.kind,installDependencies:b.installDependencies==='true',...(b.coverage==='true'?{coverage:{format:'LCOV',path:b.coveragePath}}:{})};tab='integrations';}
  ...
  app.get('/missions/:id/prepare',async(req,reply)=>{
    const sid=req.cookies.web_sid;if(!sid)return reply.redirect('/login');const {id}=req.params as {id:string};
    try{const {data:r}=await api<any>(`/api/missions/${id}/readiness`,{sid});return reply.type('text/html').send(layout('执行前准备',`<h1>${esc(r.title)}</h1><p>环境：${esc(r.environment.name)} · 配置版本 ${r.environment.revision}</p><section class="card"><h2>${r.configurationReady?'配置检查通过':'请先补齐准备条件'}</h2><ul>${r.blockers.map((v:string)=>`<li>${esc(v)}</li>`).join('')}</ul><p class="muted">${esc(r.credentialCheck)}</p></section><section class="card"><h2>执行范围与人工准备</h2><ul>${r.cases.map((c:any)=>`<li>${esc(c.title)}</li>`).join('')}</ul><ul>${r.notices.map((v:string)=>`<li>${esc(v)}</li>`).join('')}</ul><p>自动配置检查无法证明业务前置条件已经满足。请按以上范围准备测试数据，并记录需人工清理的内容。</p></section><form method="post" action="/space/${esc(r.projectId)}/start"><input type="hidden" name="missionId" value="${esc(id)}">${field('buildId','本次目标构建版本')}<button ${r.configurationReady?'':'disabled'}>开始测试</button></form><p><a href="/space/${esc(r.projectId)}?tab=integrations">修改环境配置</a> · <a href="/space/${esc(r.projectId)}?tab=missions">返回任务</a></p>`));}catch(e){return fail(reply,e);}
  });
  ...
  app.post('/cases/:id/revise',async(req,reply)=>{
    const sid=req.cookies.web_sid;if(!sid)return reply.redirect('/login');const {id}=req.params as {id:string};
    try{const {data}=await api<any>(`/api/case-versions/${id}/revise`,{sid,method:'POST',body:parseCaseForm(req.body)});return reply.redirect(`/cases/${data.id}`);}catch(e){return fail(reply,e);}
  });
