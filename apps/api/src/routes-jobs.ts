...
export function registerJobRoutes(app: FastifyInstance, prisma: PrismaClient, jobQueue: Pick<Queue, "add">) {
  ...
  app.post('/api/jobs/:id/cancel',async req=>{
    ...
    await requireProjectAccess(prisma,req,job.projectId,'LEAD');
    if(!['LOGIN_CHECK','DATA_PREPARE','DATA_CLEANUP','DATA_INSPECT','CHANGE_REVIEW','SNAPSHOT_DIFF','DOCUMENT_CHUNK','CHUNK_EXTRACT','CHUNK_BATCH','GOAL_PROPOSAL','EXPLORATION'].includes(job.kind))throw new ApiError('UNSUPPORTED','此类作业使用所属运行的取消入口');
    ...
    if(job.kind==='CHUNK_BATCH')await prisma.job.updateMany({where:{kind:'CHUNK_EXTRACT',status:{in:['QUEUED','RUNNING']},request:{path:['chunkBatchId'],equals:id}},data:{status:'CANCELLED',finishedAt:new Date()}});
    if(job.kind==='CHUNK_EXTRACT')await prisma.documentChunk.updateMany({where:{leaseOwnerJobId:id,status:'in_progress'},data:{status:'cancelled',leaseExpiresAt:null,leaseOwnerJobId:null}});
