...
export function ruleWire(row: any) {
...
export function caseWire(row: any) {
...
export async function loadReviewBundle(
  ...
  if (
    !["PARSED", "NEEDS_OCR"].includes(row.parseStatus) ||
  ...
  if (
    bundle.documentVersionId !== row.id ||
...
export async function freezeReviewInput(
  db: DB,
  store: ArtifactStore,
  projectId: string,
  baselineId: string,
  ...
  if (
    baseline.caseVersionIds.length > 500 ||
    baseline.ruleVersionIds.length > 500
  )
    throw new ApiError(
      "VALIDATION_ERROR",
      "首版变更分析最多支持 500 个规则和用例",
    );
  const old = await loadReviewBundle(db, store, projectId, oldId),
    next = await loadReviewBundle(db, store, projectId, newId);
  if (
    old.row.documentId !== next.row.documentId ||
  ...
  if (
    rules.length !== baseline.ruleVersionIds.length ||
    cases.length !== baseline.caseVersionIds.length ||
    cases.some((c) =>
      c.ruleVersionIds.some((id) => !baseline.ruleVersionIds.includes(id)),
    )
  )
    throw new ApiError("VALIDATION_ERROR", "基线存在缺失、未批准或越界资产");
  if (
    [old.row, next.row].some((r) => !["real", "mock"].includes(r.mode)) ||
    rules.some(
      (r) =>
        r.origin === "model" &&
  ...
  return {
    input,
    mode:
      [old.row, next.row].some((r) => r.mode === "mock") ||
...
export function verifyReviewOutput(input: unknown, output: unknown) {
  const i = ChangeReviewAnalysisInput.parse(input),
    o = ChangeReviewAnalysisOutput.parse(output);
  ...
  if (!source.ok || !impact.ok)
    throw new ApiError(
      ...
      "变更分析未通过来源与影响联合校验",
      { problems: [...source.problems, ...impact.problems] },
    );
  return o;
}
export function reviewTasks(output: unknown) {
  if (!output)
    return [] as { key: string; assetType: string; assetVersionId: string }[];
