...
function validateBudget(input) {
    ...
    if (!input)
        return merged;
    for (const key of ["maxToolActionsPerCase", "maxWallClockMsPerCase", "maxWallClockMsPerRun"]) {
...
export async function createRun(prisma, store, input) {
    ...
    for (const caseVersion of caseVersions) {
        ...
        if (input.pinnedPlans && !fixed)
            throw new ApiError("VALIDATION_ERROR", "原运行缺少固定计划");
        ...
        if (planVersion.acceptanceHash !== planVersion.plan.acceptanceHash) {
            throw new ApiError("VALIDATION_ERROR", "计划登记哈希与实际内容不一致");
        }
        // R4：计划观察证据（存在/同项目/OBSERVATION/文件真实存在）。
        const bindings = (planVersion.plan.bindings) ?? [];
    ...
    for (const pin of pins) {
        ...
        for (const action of plan.plan.actions)
            if (action.type === "apiCheck" && action.templateId)
                templateIds.add(action.templateId);
    }
    ...
    if (templates.length !== templateIds.size)
        throw new ApiError("VALIDATION_ERROR", "API 模板不存在或不属于目标环境");
    ...
    try {
        ...
        if (String(err).includes("Unique constraint") || err.code === "P2002") {
            const raced = await prisma.run.findUnique({
                where: { projectId_idempotencyKey: { projectId, idempotencyKey: input.idempotencyKey } },
            });
            if (raced)
                return resolveIdempotent(raced, identity, input.idempotencyKey, input.pinnedPlans);
        }
        throw err;
    }
}
...
function resolveIdempotent(existing, identity, idempotencyKey, pinnedPlans) {
    const normalizePins = (value) => JSON.stringify(value.map(p => ({ caseVersionId: p.caseVersionId, planVersionId: p.planVersionId, acceptanceHash: p.acceptanceHash })).sort((a, b) => a.caseVersionId.localeCompare(b.caseVersionId)));
    if (pinnedPlans && normalizePins(existing.casePlanPins) !== normalizePins(pinnedPlans))
        throw new ApiError("IDEMPOTENCY_CONFLICT", "相同幂等键对应不同的复测计划");
    ...
    const same = JSON.stringify(existingIdentity) === JSON.stringify({ ...identity, budget: budgetFingerprint(identity.budget) });
    if (same)
